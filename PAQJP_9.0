...
The repeat count for transforms 01,04,07,08,09 can be configured via
repeat_count= parameter (default 100).
"""
...
PROGNAME = "PAQJP_9.0_LOSSLESS_AUTO_SAFE"
...
# ---------- Constant Diapason: nibble → variable‑length codeword (prefix‑free) ----------
...
# Build inverse table (prefix tree) for decoding
...
class PAQJPCompressor:
    def __init__(self, repeat_count: int = 100):
        """
        repeat_count: number of iteration passes used in transforms
        01, 04, 07, 08, and 09 (default 100, set to 1000 for deeper mixing).
        """
    ...
    # ------------------------------------------------------------------
    # Transforms 01‑21 (adapted to use self.repeat_count)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Transform 23: Constant Diapason (whole‑file nibble‑to‑variable‑length code)
    # ------------------------------------------------------------------
    def transform_23(self, data: bytes) -> bytes:
        """Apply Constant Diapason nibble‑to‑variable‑length code to the whole file."""
        if not data:
            return b''
        bits = []
        for byte in data:
        ...
        padding_info = (4 - remaining_bits) % 4   # 0..3
    ...
    def reverse_transform_23(self, data: bytes) -> bytes:
        """Reverse the Constant Diapason transform."""
        if not data:
            return b''
        bits = []
        for byte in data:
        ...
        if safe:
            ...
            if marker == b'N':
                ...
                try:
                    ...
                    pass
            return None
        # marker‑free: try zstd, then paq, then check raw
        if HAS_ZSTD:
            try:
    ...
    # ------------------------------------------------------------------
    # Exhaustive self‑test (now includes Constant Diapason)
    # ------------------------------------------------------------------
...
def main():
    ...
    # By default we use 100 repeats – change to 1000 if desired
    c = PAQJPCompressor(repeat_count=100)   # <-- modify to 1000 here for deeper mixing
