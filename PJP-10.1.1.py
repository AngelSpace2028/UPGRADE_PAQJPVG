...
# -*- coding: utf-8 -*-
"""
Unified PAQJP+PJP – Simplified (Lossless, 256 transforms + FC)
==============================================================
- 256 base transforms + new transform 0 (FC) = 257 transforms total
- Time‑limited modes (default 300s)
- Final verification + fallback to raw+backend
- Quantum‑inspired transforms (optional)
- Exhaustive self‑test for all transforms
- Output naming: input.txt.pjp1 (or .pjp1.lzh)
- ALL transforms are individually lossless for every input.
- Composition of lossless transforms is lossless (not used here).
- zstandard and paq are optional; if missing, compression falls back to raw/LZH.
"""
...
class UnifiedCompressor:
    ...
    # ------------------------------------------------------------------
    # Backend compression with magic stripping
    # ------------------------------------------------------------------
    def _compress_backend_with_flag(self, data: bytes) -> bytes:
        ...
        if paq is not None:
            try:
                p = paq.compress(data)
                # strip 4‑byte magic 00 63 00 00 (assuming this is PAQ's header)
    ...
    def _decompress_backend_with_flag(self, data: bytes) -> bytes:
        ...
        if flag == 0:
            ...
            # re‑add paq magic
            full = b'\x00\x63\x00\x00' + payload
            return paq.decompress(full)
        else:
            raise TransformError("Backend flag invalid or backend not available")

    # ------------------------------------------------------------------
    # PAQJP transforms 28‑30 (using flagged backend)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # NEW TRANSFORM 0 (FC) – XOR with 0xFF
    # ------------------------------------------------------------------
    def transform_0(self, data: bytes) -> bytes:
        return bytes(b ^ 0xFF for b in data)

    def reverse_transform_0(self, data: bytes) -> bytes:
        return self.transform_0(data)   # self‑inverse

    # ------------------------------------------------------------------
    # Build transform maps (includes transform 0)
    # ------------------------------------------------------------------
    def _build_transform_maps(self):
        ...
        # transform 0
        self.fwd_transforms[0] = self.transform_0
        self.rev_transforms[0] = self.reverse_transform_0
    ...
    # ------------------------------------------------------------------
    # Variable‑length header encoding for single transforms (includes 0)
    # ------------------------------------------------------------------
    def _encode_marker_single(self, t: int) -> bytes:
        if t == 0:
            return bytes([253])          # unused marker for transform 0
        elif t <= 252:
            return bytes([t - 1])
        elif t <= 255:
    ...
    # ------------------------------------------------------------------
    # Main compression/decompression methods (use flagged backend)
    # ------------------------------------------------------------------
    def compress_with_verification(self, data: bytes, time_limit: Optional[float] = None) -> bytes:
        ...
        def try_candidate(header: bytes, transformed: bytes):
            ...
            candidate = header + self._compress_backend_with_flag(transformed)
            if len(candidate) < best_len:
                best_candidate = candidate
                best_len = len(candidate)

        try_candidate(self._encode_marker_raw(), data)
        for t in range(0, 257):   # include transform 0
        ...
        if best_candidate is None:
            best_candidate = self._encode_marker_raw() + self._compress_backend_with_flag(data)

        decomp, _ = self._decompress_auto(best_candidate)
        if decomp == data:
            return best_candidate

        fallback = self._encode_marker_raw() + self._compress_backend_with_flag(data)
    ...
    def _decompress_lzh_pipeline(self, data: bytes) -> Optional[bytes]:
        ...
        if payload[0] == 0xFF:
            ...
            transformed = self._decompress_backend_with_flag(payload)
            if transformed is None: return None
        if not seq:
            return transformed
    ...
    def _decompress_auto(self, data: bytes) -> Tuple[bytes, Optional[Tuple[int, ...]]]:
        ...
        res = self._decompress_backend_with_flag(payload)
        if res is None:
    ...
    # File I/O
    # ------------------------------------------------------------------
    def _auto_output_name(self, infile: str, suffix: str = ".pjp1") -> str:
    ...
    def decompress_file(self, infile: str, outfile: str = "") -> bool:
        ...
        if not outfile:
            base = os.path.basename(infile)
            name_without_suffix = re.sub(r'\.pjp1(\.lzh)?$', '', base)
    ...
    # ------------------------------------------------------------------
    # Full self‑test (now includes transform 0)
    # ------------------------------------------------------------------
    def full_self_test(self) -> bool:
        print("=" * 60)
        print("Unified PAQJP+PJP – Self‑Test (256 transforms + FC)")
        print("=" * 60)
        print("Testing all transforms on multiple byte values...")
        ...
        if all_ok:
            print("\n  All transforms passed on test bytes.")
...
# ------------------------------------------------------------
def main():
    print(f"{PROGNAME} – Unified compression with 256 transforms + FC")
    print("Compressed output: input.txt.pjp1 (or input.txt.pjp1.lzh)")
    print("Lossless output guaranteed via per-input verification and raw fallback.\n")
    c = UnifiedCompressor()

    print("=" * 58)
    print("Options:")
    print("  1) Compress (Fast)      – 257 transforms + backend")
    print("  2) Decompress")
    print("  3) Full self‑test       – verify all transforms")
    print("  0) Exit")
    print("=" * 58)
    ...
    while True:
        ...
        if choice == "1":
            ...
            while True:
                infile = input("Compressed file (.pjp1 or .pjp1.lzh) [press Enter to cancel]: ").strip()
                if not infile:
                    break
                if not (infile.lower().endswith('.pjp1') or infile.lower().endswith('.pjp1.lzh')):
                    print("Error: Incorrect format. Only .pjp1 and .pjp1.lzh files are supported.")
                    continue
