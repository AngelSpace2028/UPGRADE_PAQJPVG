...
Quantum extensions:
- Fast mode (256 transforms) uses 9‑qubit reversible circuits.
- Ultra mode (65536 transforms) uses 17‑qubit reversible circuits.
All circuits are classical reversible (X, CNOT, Toffoli, SWAP) and are simulated
deterministically using pure classical bit‑array simulation (no Aer/backend needed).
"""
...
# ---------- Quantum circuit (Qiskit needed only for circuit description, no simulation) ----------
try:
...
PROGNAME = "PAQJP_9.3_Transform65535_LZH_Quantum"
...
class PAQJPCompressorTransform65535:
    def __init__(self, repeat_count: int = 100):
        ...
        # Quantum readiness (no simulation backend needed)
    ...
    # ------------------------------------------------------------------
    # Quantum extensions – pure classical simulation of reversible circuits
    # ------------------------------------------------------------------
    @staticmethod
    ...
    def _generate_random_reversible_circuit(self, qubits: int, depth: int, seed: int) -> QuantumCircuit:
        """Generate a random classical reversible circuit using X, CNOT, Toffoli, SWAP."""
    ...
    def _apply_qc_to_block(self, bits: List[int], qc: QuantumCircuit) -> List[int]:
        """
        Classical simulation of a reversible quantum circuit on a bit string.
        Works by iterating over the circuit's operations and applying the corresponding
        bit transformations: X flips the bit, CX toggles target if control is 1,
        CCX toggles target if both controls are 1, SWAP swaps two bits.
        """
        if not HAS_QISKIT:
            raise ImportError("Qiskit is required even for classical simulation (for circuit description).")
        n = qc.num_qubits
        assert len(bits) == n
        state = list(bits)  # copy
    ...
    def _quantum_transform_data(self, data: bytes, circuit_index: int, qubits: int,
        ...
        # Convert back to bytes (ignore trailing bits if not multiple of 8)
        out_bytes = bytearray()
        for i in range(0, len(transformed_bits), 8):
            byte_val = self._bits_to_int(transformed_bits[i:i+8])
            out_bytes.append(byte_val)
        return bytes(out_bytes[:len(data)])  # truncate to original length

    # ------------------------------------------------------------------
    # Quantum compression pipelines
    # ------------------------------------------------------------------
    def compress_quantum(self, data: bytes, ultra: bool = False) -> bytes:
        """Compress using quantum reversible circuits + backend."""
        if not self.qiskit_avail:
        ...
        for idx in range(max_index + 1):
            try:
                ...
                # Encode header: marker 0xFF, then qubits flag (0x09 or 0x11), then index as 2 bytes (or 1 for fast)
                if ultra:
        ...
        if best_bytes is None:
            # Fallback: store raw data with a raw marker
    ...
    def decompress_file(self, infile: str, outfile: str):
        ...
        offset, seq = self._decode_header(data)
        if offset == 0:
            # Try quantum decompression if header starts with 0xFF
