...
class UnifiedCompressor:
    ...
    # ------------------------------------------------------------------
    # Compression backends (modified to use flag, stripping zstd magic and optional paq prefix/suffix)
    # ------------------------------------------------------------------
    def _compress_backend(self, data: bytes) -> bytes:
        ...
        if HAS_ZSTD:
            try:
                ...
                # Remove the 4-byte zstd magic (0x28 0xB5 0x2F 0xFD)
                if len(compressed) >= 4 and compressed[:4] == b'\x28\xb5\x2f\xfd':
                    compressed = compressed[4:]
                candidates.append((1, compressed))
            except:
                pass
        # paq
        if paq is not None:
            try:
                paq_data = paq.compress(data)
                # Check for exact pattern: starts with 00 63 00 00 and ends with FF FF FF
                if len(paq_data) >= 7 and paq_data[:4] == b'\x00\x63\x00\x00' and paq_data[-3:] == b'\xff\xff\xff':
                    stripped = paq_data[4:-3]
                    candidates.append((3, stripped))
                else:
                    candidates.append((2, paq_data))
            except:
                pass
        best_flag, best_data = min(candidates, key=lambda x: len(x[1]))
        return bytes([best_flag]) + best_data
    ...
    def _decompress_backend(self, data: bytes) -> Optional[bytes]:
        ...
        if flag == 0:
            ...
            # Re-add zstd magic
            full_payload = b'\x28\xb5\x2f\xfd' + payload
            try:
                return zstd_dctx.decompress(full_payload)
            except:
                return None
        elif flag == 2 and paq is not None:
            try:
                ...
                return None
        elif flag == 3 and paq is not None:
            # Re-add stripped bytes
            full_payload = b'\x00\x63\x00\x00' + payload + b'\xff\xff\xff'
            try:
                return paq.decompress(full_payload)
            except:
                return None
        else:
            return None
