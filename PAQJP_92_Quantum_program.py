...
class PAQJPCompressorTransform65535:
    ...
    # ------------------------------------------------------------------
    # Huffman coding helpers (fixed to avoid tuple vs int comparison)
    # ------------------------------------------------------------------
    @staticmethod
    ...
    # ------------------------------------------------------------------
    # Transform 47 – PAQ state table minus 400 XOR (position‑based, self‑inverse)
    # ------------------------------------------------------------------
    def transform_47(self, data: bytes) -> bytes:
        if not data: return b''
        t = bytearray(data)
        table_len = len(self.mod_state_table)
        # Safety: if table_len is 0, fall back to identity
        if table_len == 0:
            return data
        for i in range(len(t)):
