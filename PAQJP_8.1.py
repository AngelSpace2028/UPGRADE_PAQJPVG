...
# -*- coding: utf-8 -*-
"""
PAQJP 8.1 – modified transform 0 only (better random data attempt)
Tries all 256 possible shifts → picks the one that maximizes total run length
Then applies that shift + RLE with max run 16777215 (24-bit)

Only transform_00 & reverse_transform_00 are changed.
All other parts are identical to your posted code.
"""
...
PROGNAME = "PAQJP_8.1_MOD_TRANSFORM0_MAXRUN"
...
class PAQJPCompressor:
    ...
    def _peek_bits(self, bits: List[int], pos: int, count: int) -> int:
        val = 0
    ...
    def _read_bits(self, bits: List[int], pos: int, count: int) -> int:
        return self._peek_bits(bits, pos, count)

    # ────────────────────────────────────────────────
    # Modified transform 0 – tries all 256 shifts → picks best run length
    # ────────────────────────────────────────────────
    def transform_00(self, data: bytes) -> bytes:
        if not data:
            return b'\x00'

        best_data = bytearray(data)
        best_score = 0
        best_shift = 0

        # Try all 256 possible shifts
        for shift in range(256):
            ...
            # Count total run length after shift
            while i < n:
                val = tmp[i]
                run = 1
                i += 1
                while i < n and tmp[i] == val:
                    run += 1
                    i += 1
                score += run
        ...
        # Now encode the best shifted data with long RLE
        out_bits: List[int] = []

        # Header: version 010 + best shift (8 bits)
        self._append_bits(out_bits, 0b010, 3)
        self._append_bits(out_bits, best_shift, 8)
        ...
        while i < n:
            ...
            run = 1
            i += 1
            while i < n and best_data[i] == val and run < 0xFFFFFF:
                run += 1
                i += 1

            self._append_bits(out_bits, val, 8)
            self._append_bits(out_bits, run, 24)

        # Pad
        pad = (8 - len(out_bits) % 8) % 8
        self._append_bits(out_bits, 0, pad)
        ...
        for j in range(0, len(out_bits), 8):
            ...
            for k in range(8):
                if j + k < len(out_bits):
                    byte = (byte << 1) | out_bits[j + k]
            out_bytes.append(byte)

        return bytes(out_bytes) if out_bytes else b'\x00'
    ...
    def reverse_transform_00(self, cdata: bytes) -> bytes:
        ...
        # Read header: version (3) + shift (8)
        pos += 3
        shift = self._read_bits(bits, pos, 8)
        pos += 8

        while pos + 32 < nbits:
            val = self._read_bits(bits, pos, 8)
            pos += 8
            run = self._read_bits(bits, pos, 24)
            pos += 24
    ...
    # ────────────────────────────────────────────────
    # All other transforms 1–15 (exactly as in your original code)
    # ────────────────────────────────────────────────
    ...
    # ────────────────────────────────────────────────
    # Main compression logic – tries all transforms
    # ────────────────────────────────────────────────
...
def main():
    print(f"{PROGNAME} – Automatic paq + zstandard hybrid")
    print("Transform 0 modified: best shift + long RLE\n")
    c = PAQJPCompressor()
