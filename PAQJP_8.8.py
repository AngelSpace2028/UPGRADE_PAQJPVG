...
(Auto‑correcting backends – tries marker‑free first, falls back to safe markers)
----------------------------------------------------------------------------
...
BACKEND COMPRESSION (dual mode):
   Marker‑free (default):
     zstd output  : just the raw zstd stream
     paq  output  : just the raw paq stream
...
class PAQJPCompressor:
    ...
    def _decompress_backend(self, data: bytes, safe: bool = False) -> Optional[bytes]:
        ...
        # Marker‑free (original safe order)
        # 1. Try Zstd
        if HAS_ZSTD:
            try:
    ...
    # ------------------------------------------------------------------
    # Variable‑length header encoding / decoding (unchanged)
    # ------------------------------------------------------------------
    ...
    def compress_with_best(self, data: bytes, safe: bool = False) -> bytes:
        if not data:
            ...
            # verify empty
            if not safe:
                decomp, _ = self.decompress_with_best(compressed, safe=False)
                if decomp != b'':
                    # fallback to safe
        ...
        # Verify
        decomp, _ = self.decompress_with_best(best_bytes, safe)
        if decomp != data:
            if not safe:
                ...
                # Even safe mode failed (should never happen)
                raise RuntimeError("Safe compression failed – unexpected internal error!")

        return best_bytes

    def decompress_with_best(self, data: bytes, safe: bool = False) -> Tuple[bytes, Optional[Tuple[int, ...]]]:
        offset, seq = self._decode_header(data)
        if offset == 0:
            return b'', None
        payload = data[offset:]
        backend = self._decompress_backend(payload, safe)
        if backend is None:
            return b'', None
        try:
            if not seq:
    ...
    def full_self_test(self) -> bool:
        ...
        for safe in [False, True]:
            ...
            decomp_empty, _ = self.decompress_with_best(compressed_empty, safe)
            if decomp_empty != b'':
    ...
    # ------------------------------------------------------------------
    # File API (always uses the auto‑correcting compressor)
    # ------------------------------------------------------------------
    def compress_file(self, infile: str, outfile: str):
        ...
        # compress_with_best will automatically fallback if needed
    ...
    def decompress_file(self, infile: str, outfile: str):
        ...
        # Determine safe mode automatically by trying both? The file doesn't store
        # whether it was compressed in safe or marker‑free mode.
        # We can try decompression with marker‑free first; if it fails or gives ambiguous
        # result, try safe. Safer: try safe first (since safe mode always reads marker).
        # We'll try safe first, then marker‑free if safe fails.
        original, seq = self.decompress_with_best(data, safe=True)
        if original == b'' and seq is None:
            # Attempt marker‑free decompression
            original, seq = self.decompress_with_best(data, safe=False)
        if original == b'' and seq is None:
