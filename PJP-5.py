...
+ Backend markers: 0xFD = paq, 0xFE = zstd, 0xFC = raw.
+ Options 1,2,3,4,8 strip the first 4 bytes of output; header saved as .hdr file.
+ Decompression automatically prepends the .hdr if present.
============================================================================
"""
...
class PJPCompressor:
    ...
    # ------------------------------------------------------------------
    # Transform 31 – .docx paragraph extraction (NON-BIJECTIVE)
    # ------------------------------------------------------------------
    ...
    def _compress_backend(self, data: bytes, safe: bool = False) -> bytes:
        ...
        if safe:
            candidates.append((b'\xFC', b'\xFC' + data))   # fixed: now includes marker
        else:
            candidates.append((b'N', data))     # marker‑free raw
    ...
    def _decompress_backend(self, data: bytes, safe: bool = False) -> Optional[bytes]:
        ...
        if safe:
            marker = data[0:1]
            payload = data[1:]
            if marker == b'\xFE' and HAS_ZSTD:
                try:
                    return zstd_dctx.decompress(payload)
                except:
                    pass
            elif marker == b'\xFD' and paq is not None:
                try:
    ...
    def _encode_marker_raw(self) -> bytes:
        return bytes([252])   # 0xFC – indicates no transform
    ...
    def compress_with_best(self, data: bytes, safe: bool = False, ultra: bool = True,
        ...
        if not data:
            # Empty: just store raw marker + empty data (no backend compression)
            compressed = self._encode_marker_raw() + b''
            # Verify round‑trip (should always match)
        ...
        for t in range(1, 257):
            if t in (22, 23, 24, 25, 26, 27, 31, 32):
                continue
            if not include_28 and t == 28:
                continue
            if not include_29 and t == 29:
                continue
        ...
        # ---- Raw candidate: always store raw data without backend compression ----
        raw_candidate = self._encode_marker_raw() + data
        if len(raw_candidate) < best_total:
            best_total = len(raw_candidate)
            best_bytes = raw_candidate
        ...
        if decomp != data:
            if not safe:
                ...
                # Fallback to raw (no compression)
                raw_candidate = self._encode_marker_raw() + data
    ...
    def _decompress_auto(self, data: bytes) -> Tuple[bytes, Optional[Tuple[int, ...]]]:
        ...
        # If no transform (raw), payload is the final data (no backend decompression)
        if not seq:
            return payload, seq

        # Otherwise, payload must be backend‑compressed
        first_byte = payload[0:1]
        if first_byte in (b'\xFD', b'\xFE', b'\xFC'):
    ...
    # ------------------------------------------------------------------
    # File API – compression with optional stripping
    # ------------------------------------------------------------------
    def compress_file(self, infile: str, outfile: str, ultra: bool = True, hybrid: bool = False,
        ...
        # If stripping is requested, save the header to a separate file
        if strip_front > 0 and len(best_bytes) >= strip_front:
            header = best_bytes[:strip_front]
            body = best_bytes[strip_front:]
            # Write header to .hdr file
            hdr_file = outfile + ".hdr"
            try:
                with open(hdr_file, 'wb') as fh:
                    fh.write(header)
                print(f"Saved header ({strip_front} bytes) to {hdr_file}")
            except Exception as e:
                print(f"Error writing header file: {e}")
                return
            # Write body to main file
            try:
                ...
                return
            print(f"Compressed {len(data)} → {len(body)} bytes (header saved separately) ({best_method}) → {outfile}")
        else:
            try:
                with open(outfile, 'wb') as f:
                    f.write(best_bytes)
    ...
    def decompress_file(self, infile: str, outfile: str):
        ...
        # Check if there is a .hdr file (stripped header)
        hdr_file = infile + ".hdr"
        if os.path.exists(hdr_file):
            try:
                with open(hdr_file, 'rb') as fh:
                    header = fh.read()
                # Prepend header to data
                data = header + data
                print(f"Prepended header from {hdr_file}")
            except Exception as e:
                print(f"Error reading header file: {e}")
                return

        # ---------- Handle old 4‑byte magic headers ----------
        # Paq: 00 63 00 00
        if len(data) >= 4 and data[:4] == b'\x00c\x00\x00':
            print("Detected old paq magic (00 63 00 00) – stripping and decompressing with paq...")
            if paq is None:
                print("Error: paq backend not available.")
                return
            try:
                decompressed = paq.decompress(data[4:])
                original, seq = self._decompress_auto(decompressed)
                if original is None:
                    print("Decompression failed – transform error.")
                    return
                with open(outfile, 'wb') as f:
                    f.write(original)
                ...
                print(f"Error decompressing paq data: {e}")
                return

        # Zstd: 28 B5 2F FD
        if len(data) >= 4 and data[:4] == b'(\xb5/\xfd':
            print("Detected old zstd magic (28 B5 2F FD) – stripping and decompressing with zstd...")
            if not HAS_ZSTD:
                print("Error: zstd backend not available.")
                return
            try:
                decompressed = zstd_dctx.decompress(data[4:])
                original, seq = self._decompress_auto(decompressed)
                if original is None:
                    print("Decompression failed – transform error.")
                    return
        ...
        # ---------- New format with single‑byte markers ----------
